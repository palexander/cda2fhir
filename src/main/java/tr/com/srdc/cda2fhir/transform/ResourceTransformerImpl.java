...
//		}
    ...
    public Bundle tCoverageActivity2CoverageStatement(CoverageActivity coverageAct) {
        if (coverageAct == null || coverageAct.isSetNullFlavor()) return null;

        Coverage coverage = new Coverage();

        // resource id
        IdDt resourceId = new IdDt("Coverage", getUniqueId());
        coverage.setId(resourceId);

        // meta.profile
        if (Config.isGenerateDafProfileMetadata())
            coverage.getMeta().addProfile(Constants.PROFILE_DAF_CONDITION);

        // id -> identifier
        for (II id : coverageAct.getIds()) {
            if (!id.isSetNullFlavor()) {
                coverage.addIdentifier(dtt.tII2Identifier(id));
            }
        }

        // type of insurance 2.16.840.1.113883.10.20.22.4.61 code
        for (EntryRelationship rel : coverageAct.getEntryRelationships()) {
            if (!rel.isSetNullFlavor()) {
                CD code = rel.getAct().getCode();
                CodingDt newCode = new CodingDt();
        ...
        // participant
        for (Participant2 participant : coverageAct.getParticipants()) {
            for (II partRoleId : participant.getParticipantRole().getIds()) {
                if (!partRoleId.isSetNullFlavor()) {
                    coverage.addIdentifier(dtt.tII2Identifier(partRoleId));
                }
            }
        }

        // performer
        for (Performer2 performer : coverageAct.getPerformers()) {
            if (!performer.isSetNullFlavor()) {
                ...
                for (II ii : assignedEntity.getIds()) {
                    patient.addIdentifier(dtt.tII2Identifier(ii));
                }
                coverage.setSubscriber(new ResourceReferenceDt(patient));
            }

        }

        Bundle bundle = new Bundle();
        bundle.addEntry(new Bundle.Entry().setResource(coverage));
        return bundle;
    }
}